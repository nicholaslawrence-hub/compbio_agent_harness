  ...
  return (
    <div className="space-y-3">
      ...
      <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-pharma-600 to-bio-500 rounded-full transition-all duration-700"
      ...
      <div className="flex justify-between">
        {STAGES.map(s => (
          <div key={s.at} className="flex flex-col items-center gap-1">
            <div className={`w-2 h-2 rounded-full transition-colors ${progress >= s.at ? 'bg-pharma-500' : 'bg-gray-700'}`} />
            <span className={`text-xs hidden sm:block ${progress >= s.at ? 'text-gray-400' : 'text-gray-700'}`}>
              {s.label}
            </span>
          </div>
        ))}

...
export const AGENT_CARDS = [
  { type: 'count_matrix_input', label: 'Count Matrix', category: 'Entry', icon: FileSpreadsheet, inputs: [], outputs: ['counts'], requires: ['Raw count table'], returns: ['count_matrix_path'], feeds: ['DGE'], doc: 'Upload raw counts. This anchors RNA differential expression and downstream target nomination.' },
  { type: 'clinical_metadata', label: 'Clinical Metadata', category: 'Entry', icon: ClipboardList, inputs: [], outputs: ['metadata'], requires: ['sample annotations'], returns: ['covariates'], feeds: ['TCGA Survival', 'Spatial TME'], doc: 'Adds sample-level context such as cohort, tumor purity, batch, and clinical phenotype.' },
  { type: 'study_context', label: 'Study Directive', category: 'Entry', icon: BrainCircuit, inputs: [], outputs: ['directive'], requires: ['free text goal'], returns: ['global prompt'], feeds: ['Supervisor', 'Critics'], doc: 'Injects user goals into supervisor and critic decisions.' },
  { type: 'sync_gateway', label: 'Sync Gateway', category: 'Control', icon: GitBranch, inputs: ['clinical_signal', 'docking', 'structure_signal', 'chemistry_profile'], outputs: ['translation_brief'], requires: ['multiple upstream nodes'], returns: ['joined evidence brief'], feeds: ['downstream gated node'], doc: 'Waits for all upstream branches before allowing the graph to continue.' },
  { type: 'approval_gate', label: 'Approval Gate', category: 'Control', icon: ShieldAlert, inputs: ['translation_brief'], outputs: ['decision'], requires: ['candidate decision packet'], returns: ['human approval request'], feeds: ['expensive downstream work'], doc: 'Pauses the graph before costly or irreversible work and asks the user to approve or reroute.', wip: true },
  { type: 'run_dge', label: 'DGE', category: 'RNA', icon: FlaskConical, inputs: ['counts'], outputs: ['degs', 'genes', 'signature'], requires: ['counts', 'case/control'], returns: ['DEGs'], feeds: ['VIPER', 'SCENIC', 'TCGA'], doc: 'Runs PyDESeq2 differential expression. Produces ranked genes and adjusted p-values.' },
  { type: 'viper_protein_activity', label: 'VIPER Activity', category: 'Target Validation', icon: Activity, inputs: ['degs', 'genes'], outputs: ['regulator_activity'], requires: ['DE stats', 'DoRothEA regulons'], returns: ['TF activity NES'], feeds: ['Supervisor', 'SCENIC'], doc: 'Infers regulator activity from expression using VIPER logic and DoRothEA regulons. Useful when RNA abundance does not equal protein activity.', wip: true },
  { type: 'mageck_crispr', label: 'MAGeCK CRISPR', category: 'Target Validation', icon: Dna, inputs: ['genes'], outputs: ['essentiality_score'], requires: ['sgRNA count table', 'design matrix'], returns: ['beta scores'], feeds: ['DepMap', 'Critics'], doc: 'Models CRISPR screen essentiality using MAGeCK MLE beta-score concepts.', wip: true },
  { type: 'scenic_regulon', label: 'SCENIC Regulon', category: 'Systems', icon: Waypoints, inputs: ['counts', 'degs'], outputs: ['regulon_program'], requires: ['expression matrix'], returns: ['master regulators'], feeds: ['VIPER', 'Report'], doc: 'Identifies regulator programs that can explain downstream expression structure.', wip: true },
  { type: 'spatial_tme', label: 'Spatial TME', category: 'Systems', icon: Layers3, inputs: ['genes', 'metadata'], outputs: ['spatial_context'], requires: ['spatial cohort'], returns: ['tumor nest localization'], feeds: ['TME Critic'], doc: 'Checks whether the target localizes to the right cell compartment and tumor microenvironment.', wip: true },
  { type: 'lincs_reversion', label: 'LINCS Reversion', category: 'Perturbation', icon: Beaker, inputs: ['signature'], outputs: ['perturbation_match', 'smiles'], requires: ['disease signature'], returns: ['reversal molecules'], feeds: ['Drug Annotation'], doc: 'Searches for perturbations predicted to reverse the disease transcriptional state.', wip: true },
  { type: 'tcga_survival', label: 'TCGA Survival', category: 'Clinical', icon: Activity, inputs: ['genes', 'metadata'], outputs: ['clinical_signal'], requires: ['gene', 'cohort'], returns: ['KM p-value'], feeds: ['Report'], doc: 'Tests whether expression tracks patient outcome using Kaplan-Meier style survival logic.', wip: true },
  { type: 'pharmacogenomics_pgx', label: 'PharmGKB PGx', category: 'Clinical', icon: Pill, inputs: ['protein', 'smiles'], outputs: ['clinical_signal'], requires: ['target or drug'], returns: ['allele risk'], feeds: ['Report'], doc: 'Screens population pharmacogenomic toxicity liabilities.', wip: true },
  { type: 'enrich_ppi', label: 'STRING PPI', category: 'Network', icon: Network, inputs: ['genes', 'regulator_activity', 'clinical_signal'], outputs: ['protein', 'network_context'], requires: ['gene list'], returns: ['partners'], feeds: ['AlphaFold3', 'GNINA'], doc: 'Finds interaction partners. Biologically, this is where undruggable primary targets can become druggable network neighbors.' },
  { type: 'alphafold_complex', label: 'AlphaFold DB', category: 'Structure', icon: Atom, inputs: ['protein', 'genes'], outputs: ['structure', 'structure_signal'], requires: ['UniProt accession or gene'], returns: ['AlphaFold DB structure pointer'], feeds: ['Structural Critic'], doc: 'Resolves targets to AlphaFold DB metadata and structure download URLs. RNAgent does not run AlphaFold locally.' },
  { type: 'evo2_fitness', label: 'Evo 2 Fitness', category: 'Foundation', icon: Dna, inputs: ['protein'], outputs: ['structure_signal'], requires: ['sequence variants'], returns: ['fitness landscape'], feeds: ['ESM3'], doc: 'API adapter slot for sequence fitness annotations. No local foundation model is executed.', wip: true },
  { type: 'esm3_design', label: 'ESM3 Design', category: 'Foundation', icon: Sparkles, inputs: ['protein', 'structure'], outputs: ['structure_signal'], requires: ['protein context'], returns: ['binder design constraints'], feeds: ['GNINA'], doc: 'API adapter slot for protein design constraints. No local ESM3 model is executed.', wip: true },
  { type: 'reinvent_generative', label: 'REINVENT', category: 'Chemistry', icon: Sparkles, inputs: ['pocket', 'structure'], outputs: ['smiles'], requires: ['target pocket'], returns: ['SMILES'], feeds: ['RDKit', 'GNINA'], doc: 'External API adapter slot for de novo SMILES generation.', wip: true },
  { type: 'rdkit_features', label: 'RDKit ADMET', category: 'Chemistry', icon: Beaker, inputs: ['smiles'], outputs: ['ligand', 'chemistry_profile'], requires: ['SMILES'], returns: ['Morgan fingerprint', 'Lipinski'], feeds: ['GNINA', 'Report'], doc: 'Parses generated or known SMILES and computes compact lead-like property records.' },
  { type: 'gnina_docking', label: 'GNINA Docking', category: 'Chemistry', icon: ScanSearch, inputs: ['structure', 'ligand', 'smiles'], outputs: ['docking', 'chemistry_profile'], requires: ['PDB', 'SDF'], returns: ['CNNscore', 'CNNaffinity'], feeds: ['Structural Critic'], doc: 'External docking adapter slot. RNAgent does not run local GNINA by default.', wip: true },
  { type: 'drug_annotation', label: 'Drug Annotation', category: 'Chemistry', icon: Pill, inputs: ['genes', 'protein'], outputs: ['smiles', 'chemistry_profile'], requires: ['gene'], returns: ['ChEMBL', 'UniProt'], feeds: ['PGx'], doc: 'Checks existing druggability and target annotation through UniProt and ChEMBL APIs.' },
  { type: 'critic_structural_tractability', label: 'Structural Critic', category: 'Critic', icon: ShieldAlert, inputs: ['structure', 'docking', 'structure_signal', 'chemistry_profile'], outputs: ['decision'], requires: ['structure or docking'], returns: ['retry or forward'], feeds: ['upstream retry', 'next gate'], doc: 'Rejects targets with weak structural tractability, poor docking, or disorder risk.' },
  { type: 'critic_microenvironment_validity', label: 'TME Critic', category: 'Critic', icon: ShieldAlert, inputs: ['spatial_context'], outputs: ['decision'], requires: ['spatial evidence'], returns: ['retry or forward'], feeds: ['upstream retry', 'next gate'], doc: 'Rejects targets in the wrong cell type or compartment.', wip: true },
  { type: 'crispr_designer', label: 'CRISPR Designer', category: 'Wet Lab', icon: Microscope, inputs: ['genes', 'decision'], outputs: ['wetlab_design'], requires: ['validated target'], returns: ['gRNA candidates'], feeds: ['Report'], doc: 'Hands validated targets to wet-lab testing by drafting CRISPR guide candidates.', wip: true },
  { type: 'literature_rag', label: 'Literature RAG', category: 'Evidence', icon: BookOpen, inputs: ['genes', 'network_context', 'clinical_signal'], outputs: ['literature_support'], requires: ['target query'], returns: ['PMIDs'], feeds: ['Critics'], doc: 'Retrieves literature evidence and dark-gene context.' },
  { type: 'supervisor', label: 'Supervisor', category: 'Control', icon: GitBranch, inputs: ['translation_brief', 'directive'], outputs: ['decision'], requires: ['history'], returns: ['next node'], feeds: ['specialists'], doc: 'Agentic router that decides next tool and query when used in flexible runs.' },
  { type: 'translator', label: 'Omics Translator', category: 'Control', icon: GitBranch, inputs: ['counts', 'degs', 'genes', 'signature', 'regulator_activity', 'essentiality_score', 'regulon_program', 'spatial_context', 'perturbation_match', 'clinical_signal', 'network_context', 'protein', 'structure', 'structure_signal', 'smiles', 'ligand', 'chemistry_profile', 'docking', 'literature_support', 'wetlab_design', 'decision'], outputs: ['genes', 'protein', 'pocket', 'smiles', 'ligand', 'translation_brief', 'decision'], requires: ['source payload'], returns: ['adapted biological payload'], feeds: ['any compatible input'], doc: 'Edge-level adapter that extracts typed biological concepts. Use deterministic parsing for IDs, symbols, and scores. Use a small LLM only when a free-text summary must be condensed into a typed brief.' },
  { type: 'report', label: 'Report', category: 'Output', icon: Save, inputs: ['translation_brief', 'clinical_signal', 'chemistry_profile', 'literature_support', 'wetlab_design', 'decision'], outputs: ['report'], requires: ['accepted evidence'], returns: ['markdown'], feeds: ['end'], doc: 'Final synthesis sink.' },
]

...
export const AGENT_CARDS = [
  ...
  { type: 'approval_gate', label: 'Approval Gate', category: 'Control', icon: ShieldAlert, inputs: ['translation_brief'], outputs: ['decision'], requires: ['candidate decision packet'], returns: ['human approval request'], feeds: ['expensive downstream work'], doc: 'Pauses the graph before costly or irreversible work and asks the user to approve or reroute.' },
  ...
  { type: 'mageck_crispr', label: 'MAGeCK CRISPR', category: 'Target Validation', icon: Dna, inputs: ['genes'], outputs: ['essentiality_score'], requires: ['sgRNA count table', 'design matrix'], returns: ['beta scores'], feeds: ['DepMap', 'Critics'], doc: 'Models CRISPR screen essentiality using MAGeCK MLE beta-score concepts.' },
  { type: 'scenic_regulon', label: 'SCENIC Regulon', category: 'Systems', icon: Waypoints, inputs: ['counts', 'degs'], outputs: ['regulon_program'], requires: ['expression matrix'], returns: ['master regulators'], feeds: ['VIPER', 'Report'], doc: 'Identifies regulator programs that can explain downstream expression structure.' },
  { type: 'spatial_tme', label: 'Spatial TME', category: 'Systems', icon: Layers3, inputs: ['genes', 'metadata'], outputs: ['spatial_context'], requires: ['spatial cohort'], returns: ['tumor nest localization'], feeds: ['TME Critic'], doc: 'Checks whether the target localizes to the right cell compartment and tumor microenvironment.' },
  { type: 'lincs_reversion', label: 'LINCS Reversion', category: 'Perturbation', icon: Beaker, inputs: ['signature'], outputs: ['perturbation_match', 'smiles'], requires: ['disease signature'], returns: ['reversal molecules'], feeds: ['Drug Annotation'], doc: 'Searches for perturbations predicted to reverse the disease transcriptional state.' },
  { type: 'tcga_survival', label: 'TCGA Survival', category: 'Clinical', icon: Activity, inputs: ['genes', 'metadata'], outputs: ['clinical_signal'], requires: ['gene', 'cohort'], returns: ['KM p-value'], feeds: ['Report'], doc: 'Tests whether expression tracks patient outcome using Kaplan-Meier style survival logic.' },
  { type: 'pharmacogenomics_pgx', label: 'PharmGKB PGx', category: 'Clinical', icon: Pill, inputs: ['protein', 'smiles'], outputs: ['clinical_signal'], requires: ['target or drug'], returns: ['allele risk'], feeds: ['Report'], doc: 'Screens population pharmacogenomic toxicity liabilities.' },
  ...
  { type: 'evo2_fitness', label: 'Evo 2 Fitness', category: 'Foundation', icon: Dna, inputs: ['protein'], outputs: ['structure_signal'], requires: ['sequence variants'], returns: ['fitness landscape'], feeds: ['ESM3'], doc: 'API adapter slot for sequence fitness annotations. No local foundation model is executed.' },
  { type: 'esm3_design', label: 'ESM3 Design', category: 'Foundation', icon: Sparkles, inputs: ['protein', 'structure'], outputs: ['structure_signal'], requires: ['protein context'], returns: ['binder design constraints'], feeds: ['GNINA'], doc: 'API adapter slot for protein design constraints. No local ESM3 model is executed.' },
  { type: 'reinvent_generative', label: 'REINVENT', category: 'Chemistry', icon: Sparkles, inputs: ['pocket', 'structure'], outputs: ['smiles'], requires: ['target pocket'], returns: ['SMILES'], feeds: ['RDKit', 'GNINA'], doc: 'External API adapter slot for de novo SMILES generation.' },
  ...
  { type: 'gnina_docking', label: 'GNINA Docking', category: 'Chemistry', icon: ScanSearch, inputs: ['structure', 'ligand', 'smiles'], outputs: ['docking', 'chemistry_profile'], requires: ['PDB', 'SDF'], returns: ['CNNscore', 'CNNaffinity'], feeds: ['Structural Critic'], doc: 'External docking adapter slot. RNAgent does not run local GNINA by default.' },
  ...
  { type: 'critic_microenvironment_validity', label: 'TME Critic', category: 'Critic', icon: ShieldAlert, inputs: ['spatial_context'], outputs: ['decision'], requires: ['spatial evidence'], returns: ['retry or forward'], feeds: ['upstream retry', 'next gate'], doc: 'Rejects targets in the wrong cell type or compartment.' },
  { type: 'crispr_designer', label: 'CRISPR Designer', category: 'Wet Lab', icon: Microscope, inputs: ['genes', 'decision'], outputs: ['wetlab_design'], requires: ['validated target'], returns: ['gRNA candidates'], feeds: ['Report'], doc: 'Hands validated targets to wet-lab testing by drafting CRISPR guide candidates.' },
